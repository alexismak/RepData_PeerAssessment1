...
```
The mean is `r paste(round(meanSt, digits=1))` and the median is `r paste(medianSt)`.
...
Total number of missing values is `r misValues`.
...
```
The new mean is `r paste(round(meanStNew, digits = 1))` (old mean was `r paste(round(meanSt, digits = 1))`) and the new median is `r paste(medianStNew)` (old median was `r paste(medianSt)`). The effect is very small, but it is towards reducing the mean and median values.
